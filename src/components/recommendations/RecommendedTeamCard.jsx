...
import { FiBriefcase, FiZap, FiCheckCircle } from 'react-icons/fi';
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all h-full flex flex-col p-6 relative overflow-hidden group">
      ...
      <div className="mb-4 flex-1">
        <h3 className="text-lg font-extrabold text-gray-900 truncate mb-1 group-hover:text-blue-600 transition-colors" title={team.name}>
      ...
      {reasons.length > 0 && (
        <div className="mb-6 space-y-1.5">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1">
            <FiZap className="w-3 h-3 text-amber-500" /> Why this team?
          </p>
      ...
      <div className="mt-auto pt-4 border-t border-gray-50 flex gap-2">
        <Link 
          to={`/teams/${team.id}`}
          className="flex-1 py-2.5 bg-gray-50 hover:bg-blue-50 text-blue-600 hover:text-blue-700 text-center font-bold text-sm rounded-xl transition-colors"
        >
          View Team
        </Link>
      </div>
