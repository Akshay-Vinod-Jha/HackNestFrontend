export default function RecommendedTeamCard({ data }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-3 gap-2">
          <h3 className="font-extrabold text-gray-900 text-lg leading-tight">{data?.name || 'Unknown Team'}</h3>
          <span className="bg-blue-50 text-blue-700 text-xs font-bold px-2.5 py-1 rounded-full whitespace-nowrap border border-blue-100 shrink-0">
      ...
      <button className="w-full mt-2 py-2.5 text-sm font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors active:scale-[0.98]">
        View Team
      </button>
    </div>
  );
